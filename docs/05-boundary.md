...
Un admin doit se connecter à un serveur de production. Il lui faut :
...
3. **Être révoqué individuellement** → si l'admin quitte l'équipe, il faut retirer sa clé de chaque serveur manuellement
...
L'admin ne connaît jamais l'IP. Il se connecte à une **target nommée**.
...
Ingénieur ──── boundary connect ──► Boundary (proxy) ──► 10.0.0.20:22
               ...
               target par nom)        révocation possible)   à l'admin)
```
...
2. boundary connect ssh       → Boundary crée une session, ouvre un proxy local (127.0.0.1:PORT_ALEATOIRE)
...
L'admin voit `127.0.0.1:45421` dans sa session SSH - jamais `10.0.0.20`.
...
1. Ajoute le repo HashiCorp et installe `boundary` via dnf (Fedora)
...
### Etape 2 - Voir les targets disponibles
...
L'admin voit les noms et les IDs - **pas forcément les IPs** (selon les permissions).

### Etape 3 - Connexion SSH via Boundary
...
### Etape 4 - Inspecter la session (depuis un autre terminal)
...
boundary sessions cancel \
  -id=s_XXXX \
  -token env://BOUNDARY_TOKEN
# → La connexion SSH de l'admin est coupée immédiatement
```
...
Je ne connais pas l'IP de ce serveur. Mon accès est une
...
# → La connexion SSH est coupée côté admin
```

En incident de sécurité, je révoque ici. Pas de clé à
